...
import com.gchc.ing.charting.data.LineDataSet;
import com.gchc.ing.charting.highlight.Highlight;
import com.gchc.ing.charting.interfaces.dataprovider.LineDataProvider;
import com.gchc.ing.charting.interfaces.datasets.IDataSet;
import com.gchc.ing.charting.interfaces.datasets.ILineDataSet;
...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    protected void drawHorizontalBezier(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            CEntry prev = dataSet.getEntryForIndex(mXBounds.min);
            CEntry cur = prev;
    ...
    protected void drawCubicBezier(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            CEntry prevPrev;
            CEntry prev = dataSet.getEntryForIndex(Math.max(firstIndex - 2, 0));
            CEntry cur = dataSet.getEntryForIndex(Math.max(firstIndex - 1, 0));
            CEntry next = cur;
            int nextIndex = -1;
    ...
    protected void drawLinear(Canvas c, ILineDataSet dataSet) {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            for (int j = mXBounds.min; j <= mXBounds.range + mXBounds.min; j++) {
                ...
                CEntry e = dataSet.getEntryForIndex(j);
                if (e == null) continue;
    ...
    private void generateFilledPath(final ILineDataSet dataSet, final int startIndex, final int endIndex, final Path outputPath) {
        ...
        final CEntry entry = dataSet.getEntryForIndex(startIndex);

        filled.moveTo(entry.getX(), fillMin);
        filled.lineTo(entry.getX(), entry.getY() * phaseY);

        // create a new path
        CEntry currentEntry = null;
        CEntry previousEntry = null;
        ...
        if (isDrawingValuesAllowed(mChart)) {
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    CEntry entry = dataSet.getEntryForIndex(j / 2 + mXBounds.min);
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight high : indices) {
            ...
            CEntry e = set.getEntryForXValue(high.getX(), high.getY());
