 ...
 */
public interface IDataSet<T extends CEntry> {
    ...
    /**
     * Calculates the min and max y-values from the CEntry closest to the given fromX to the CEntry closest to the given toX value.
    ...
    /**
     * Returns the first CEntry object found at the given x-value with binary
     * search.
     * If the no CEntry at the specified x-value is found, this method
     * returns the CEntry at the closest x-value according to the rounding.
     ...
     *                 if there is no CEntry matching the provided x-value
     * @return
     *
     *
     */
     ...
     * search.
     * If the no CEntry at the specified x-value is found, this method
     * returns the CEntry at the closest x-value.
     * INFORMATION: This method does calculations at runtime. Do
     * not over-use in performance critical situations.
     *
     *
    ...
    /**
     * Returns all CEntry objects found at the given x-value with binary
     * search. An empty array if no CEntry object at that x-value.
     * INFORMATION: This method does calculations at runtime. Do
     * not over-use in performance critical situations.
     *
     * @param xValue
    ...
    /**
     * Returns the CEntry object found at the given index (NOT xIndex) in the values array.
     *
     * @param index
     * @return
     */
    ...
    /**
     * Returns the first CEntry index found at the given x-value with binary
     * search.
     * If the no CEntry at the specified x-value is found, this method
     * returns the CEntry at the closest x-value according to the rounding.
     * INFORMATION: This method does calculations at runtime. Do
    ...
    /**
     * Returns the position of the provided entry in the DataSets CEntry array.
     ...
     * index in the CEntry array of the DataSet for a given xIndex. IMPORTANT: This method does
    ...
    /**
     * Adds an CEntry to the DataSet dynamically.
    ...
    /**
     * Removes the first CEntry (at index 0) of this DataSet from the entries array.
    ...
    /**
     * Removes the last CEntry (at index size-1) of this DataSet from the entries array.
     * Returns true if successful, false if not.
     *
     * @return
     */
    ...
    /**
     * Removes an CEntry from the DataSets entries array. This will also
     * recalculate the current minimum and maximum values of the DataSet and the
     * value-sum. Returns true if an CEntry was removed, false if no CEntry could
     * be removed.
     *
     * @param e
     */
    ...
    /**
     * Removes the CEntry object closest to the given x-value from the DataSet.
     * Returns true if an CEntry was removed, false if no CEntry could be removed.
     *
     * @param xValue
     */
    ...
    /**
     * Removes the CEntry object at the given index in the values array from the DataSet.
     * Returns true if an CEntry was removed, false if no CEntry could be removed.
     *
     * @param index
     * @return
    ...
    /**
     * Checks if this DataSet contains the specified CEntry. Returns true if so,
