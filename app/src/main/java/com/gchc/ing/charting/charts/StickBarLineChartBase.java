...
import com.gchc.ing.charting.components.XAxis.XAxisPosition;
...
import com.gchc.ing.charting.highlight.ChartHighlighter;
import com.gchc.ing.charting.highlight.Highlight;
import com.gchc.ing.charting.interfaces.dataprovider.BarLineScatterCandleBubbleDataProvider;
import com.gchc.ing.charting.interfaces.datasets.IBarLineScatterCandleBubbleDataSet;
import com.gchc.ing.charting.jobs.AnimatedMoveViewJob;
import com.gchc.ing.charting.jobs.AnimatedZoomJob;
import com.gchc.ing.charting.jobs.MoveViewJob;
import com.gchc.ing.charting.jobs.ZoomJob;
import com.gchc.ing.charting.listener.BarLineChartTouchListener;
import com.gchc.ing.charting.listener.OnDrawListener;
import com.gchc.ing.charting.renderer.XAxisRenderer;
import com.gchc.ing.charting.renderer.YAxisRenderer;
...
public class StickBarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
        IBarLineScatterCandleBubbleDataSet<? extends CEntry>>>
        extends Chart<T> implements BarLineScatterCandleBubbleDataProvider {

    private static final String TAG = BarLineChartBase.class.getSimpleName();
    /**
...
// /** the approximator object used for data filtering */
    ...
    public StickBarLineChartBase(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
    }

    public StickBarLineChartBase(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public StickBarLineChartBase(Context context) {
        super(context);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mAxisRendererLeft.renderAxisLine(canvas);   // MrSohn 왼쪽 기준선 그리기
        mAxisRendererRight.renderAxisLine(canvas);

        mXAxisRenderer.renderGridLines(canvas);
        mAxisRendererLeft.renderGridLines(canvas);    // MrSohn 뒷 라인들 그리기
        ...
        mXAxisRenderer.renderAxisLabels(canvas);          // mrsohn X값
        mAxisRendererLeft.renderAxisLabels(canvas);       // mrsohn 왼쪽 Y값
...
/** CODE BELOW IS GETTERS AND SETTERS */
     ...
     * Returns the position (in pixels) the provided CEntry has inside the chart
     * view or null, if the provided CEntry is null.
     *
     * @param e
     * @return
     */
    public MPPointF getPosition(CEntry e, AxisDependency axis) {
    ...
    /**
     * returns the CEntry object displayed at the touched position of the chart
     *
     * @param x
     * @param y
     * @return
     */
    public CEntry getEntryByTouchPoint(float x, float y) {
