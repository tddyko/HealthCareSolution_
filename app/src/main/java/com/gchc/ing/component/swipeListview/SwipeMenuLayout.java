package com.gchc.ing.component.swipeListview;
...
public class SwipeMenuLayout extends FrameLayout {
	...
	public SwipeMenuLayout(View contentView, SwipeMenuView menuView, Interpolator closeInterpolator, Interpolator openInterpolator) {
		...
		mOpenIv = (ImageView)contentView.findViewById(R.id.open_list_button);
		mOpenIv.setOnClickListener(new OnClickListener() {
			@Override
	...
	public void smoothCloseMenu() {
		...
		if (mSwipeDirection == SwipeMenuListView.DIRECTION_LEFT) {
			...
			Logger.i(TAG, "smoothCloseMenu1");
			setArrowImage(false);
		} else {
			mBaseX = mMenuView.getRight();
			mCloseScroller.startScroll(0, 0, mMenuView.getWidth(), 0, 350);
			Logger.i(TAG, "smoothCloseMenu2");
		}
		postInvalidate();
	}
	...
	public void smoothOpenMenu() {
		...
		if (mSwipeDirection == SwipeMenuListView.DIRECTION_LEFT) {
			...
			Logger.i(TAG, "smoothOpenMenu1");
			setArrowImage(true);
		} else {
			mOpenScroller.startScroll(mContentView.getLeft(), 0, mMenuView.getWidth(), 0, 350);
			Logger.i(TAG, "smoothOpenMenu2");
		}
		postInvalidate();
	}
	...
	@TargetApi(Build.VERSION_CODES.LOLLIPOP)
	private void setArrowImage(Boolean isOpen) {
		if (isOpen) {
			mOpenIv.setImageDrawable(getContext().getDrawable(R.drawable.arrow_right));
		} else {
			mOpenIv.setImageDrawable(getContext().getDrawable(R.drawable.arrow_left));
		}
	}
