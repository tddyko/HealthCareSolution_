...
public class RadarHighlighter extends PieRadarHighlighter<RadarChart> {
    ...
    protected List<Highlight> getHighlightsAtIndex(int index) {
        ...
        for (int i = 0; i < mChart.getData().getDataSetCount(); i++) {
            ...
            final ChartEntry chartEntry = dataSet.getEntryForIndex(index);

            float y = (chartEntry.getY() - mChart.getYChartMin());

            Utils.getPosition(
                    mChart.getCenterOffsets(), y * factor * phaseY,
                    sliceangle * index * phaseX + mChart.getRotationAngle(), pOut);

            mHighlightBuffer.add(new Highlight(index, chartEntry.getY(), pOut.x, pOut.y, i, dataSet.getAxisDependency()));
        }
