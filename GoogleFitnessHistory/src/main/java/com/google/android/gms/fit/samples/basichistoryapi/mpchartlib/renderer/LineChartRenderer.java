...
import com.google.android.gms.fit.samples.basichistoryapi.mpchartlib.charts.LineChart;
...
import com.google.android.gms.fit.samples.basichistoryapi.mpchartlib.data.LineDataSet;
import com.google.android.gms.fit.samples.basichistoryapi.mpchartlib.highlight.Highlight;
import com.google.android.gms.fit.samples.basichistoryapi.mpchartlib.interfaces.dataprovider.LineDataProvider;
import com.google.android.gms.fit.samples.basichistoryapi.mpchartlib.interfaces.datasets.IDataSet;
import com.google.android.gms.fit.samples.basichistoryapi.mpchartlib.interfaces.datasets.ILineDataSet;
...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    protected void drawHorizontalBezier(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            ChartEntry prev = dataSet.getEntryForIndex(mXBounds.min);
            ChartEntry cur = prev;
    ...
    protected void drawCubicBezier(ILineDataSet dataSet) {
        ...
        if (mXBounds.range >= 1) {
            ...
            ChartEntry prevPrev;
            ChartEntry prev = dataSet.getEntryForIndex(Math.max(firstIndex - 2, 0));
            ChartEntry cur = dataSet.getEntryForIndex(Math.max(firstIndex - 1, 0));
            ChartEntry next = cur;
            int nextIndex = -1;
    ...
    protected void drawLinear(Canvas c, ILineDataSet dataSet) {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            for (int j = mXBounds.min; j <= mXBounds.range + mXBounds.min; j++) {
                ...
                ChartEntry e = dataSet.getEntryForIndex(j);
                if (e == null) continue;
    ...
    private void generateFilledPath(final ILineDataSet dataSet, final int startIndex, final int endIndex, final Path outputPath) {
        ...
        final ChartEntry chartEntry = dataSet.getEntryForIndex(startIndex);

        filled.moveTo(chartEntry.getX(), fillMin);
        filled.lineTo(chartEntry.getX(), chartEntry.getY() * phaseY);

        // create a new path
        ChartEntry currentChartEntry = null;
        ChartEntry previousChartEntry = null;
        for (int x = startIndex + 1; x <= endIndex; x++) {

            currentChartEntry = dataSet.getEntryForIndex(x);

            if (isDrawSteppedEnabled && previousChartEntry != null) {
                filled.lineTo(currentChartEntry.getX(), previousChartEntry.getY() * phaseY);
            }

            filled.lineTo(currentChartEntry.getX(), currentChartEntry.getY() * phaseY);

            previousChartEntry = currentChartEntry;
        }

        // close up
        if (currentChartEntry != null) {
            filled.lineTo(currentChartEntry.getX(), fillMin);
        }
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    if (dataSet.isDrawValuesEnabled()) {
                        drawValue(c, dataSet.getValueFormatter(), chartEntry.getY(), chartEntry, i, x,
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight high : indices) {
            ...
            ChartEntry e = set.getEntryForXValue(high.getX(), high.getY());
